...
def _s_player_prop(f: MatchFacts, outcome: str, line: Optional[float]) -> Settlement:
    """
    Player count prop. `outcome` is 'over'|'under' and the player is carried
    on the bet, so this only checks the count. The count must be supplied via
    `line` semantics by the caller through player_counts - see engine.
    """
    return Settlement.UNSETTLEABLE
...
MARKET_CATALOGUE: Dict[str, MarketSpec] = {
    ...
    "european_handicap": MarketSpec(
        ...
        outcome_kind="three_way", needs_line=True, settle=_s_result,
        model="derived_from_scoreline", void_rules=("Void if abandoned",),
        notes="No push - the draw on the handicap is a third outcome",
    ),
    "spreads": MarketSpec(
...
def get_spec(market_key: str) -> Optional[MarketSpec]:
    return MARKET_CATALOGUE.get(market_key)
...
def validate_line(market_key: str, line: Optional[float]) -> Tuple[bool, str]:
    ...
    if not spec.needs_line:
        return True, ""
    if line is not None and line < 0 and spec.outcome_kind != "handicap":
...
def settle_market(market_key: str, facts: MatchFacts, outcome: str,
    ...
    if market_key in ("totals", "half_time_goals", "corners_total", "cards_total",
                      "booking_points", "shots_total", "shots_on_target_total",
                      "offsides_total"):
        return _s_total(facts, outcome, line, stat=MARKET_STAT[market_key])
